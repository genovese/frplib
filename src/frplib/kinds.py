...
class ConditionalKind:           # pylint: disable=too-many-instance-attributes
    ...
    def transform_joined(self, statistic) -> ConditionalKind:
        """Transform the joined Kinds from a conditional Kind with a compatible statistic.

        The statistic takes the joined tuples of the input the conditional Kind and
        the target values. The resulting conditional Kind has the same inputs
        and uses the transformed Kinds as *targets*. In other words, when the
        transformed conditional Kind is joined, the inputs are still prepended
    ...
    def transform_targets(self, statistic) -> ConditionalKind:
        """Transforms the targets of a conditional Kind with a compatible statistic.

        The resulting conditional Kind has the same inputs but uses the transformed
        targets.
...
def conditional_kind(
    ...
    """Converts a mapping from values to Kinds into a conditional Kind.
    ...
    codim: int | None -- the codimension of the conditional Kind,
        i.e., the dimension of the input values. If not specified
        (None), then any length tuple is accepted. If set explicitly
        to 1, then a wrapped function should accept a scalar
    ...
    The `codim`, `dim`, and `domain` arguments are used for
