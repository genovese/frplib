...
with arbitrary symbolic weights. The signature is `arbitrary(*xs, names=None)`
where `*xs` stands for zero or more arguments that are values,
which are converted to quantities automatically. If `names` is supplied
it should be a list of strings to name the symbols assigned to the weights.
By default, arbitrary names are given to those symbols. (See `gen_symbol`.)
...
Values can be specified in a variety of ways:
  ...
  + As an iterable, e.g., `arbitrary([1, 10, 20])` or `arbitrary(irange(1,52))`
    or a generator expression `arbitrary(x + 10 for x in range(9))`.
