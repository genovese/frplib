...
Because Conditional FRPs return FRPs for each input value, we can
transform them by statistics. There are three methods for doing
these transforms.

If `C` is a Conditional FRP of type 'm -> m + n', then

+ `C.transform_targets(psi)` creates a Conditional FRP of type
...
+ `C.transform_joined(phi)` creates a Conditional FRP of type
  `m -> m + r` where `phi` is a statistic of type `m + n -> r`.
  This transforms the *joined only* with the statistic `phi`.
  `psi` must be compatible with all the joined FRPs.
